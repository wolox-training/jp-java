...
public class BookRoute extends RouteBuilder {
  ...
  public void configure() throws Exception {
    ...
    rest().description("Wolox Training Book Service")
        ...
        .get("/users/{before}/{after}/{name}")
            .description("allows you to search between a range of birth dates and a name")
            .param().name("before").type(RestParamType.path).description("date before")
            .dataType("string").required(false).endParam()
            .param().name("after").type(RestParamType.path).description("date after")
            .dataType("string").required(false).endParam()
            .param().name("name").type(RestParamType.path).description("name user")
            .dataType("string").required(false).endParam()
            .responseMessage().code(200).message("OK").endResponseMessage()
            .responseMessage().code(500).message("error generating query").endResponseMessage()
        ...
        .get("/books/{publisher}/{genre}/{year}")
            .description("allows you to search for a book by publisher, genre and year")
            .param().name("publisher").type(RestParamType.path).description("publisher book")
            .dataType("string").required(false).endParam()
            .param().name("genre").type(RestParamType.path).description("genre book")
            .dataType("string").required(false).endParam()
            .param().name("year").type(RestParamType.path).description("year book")
            .dataType("string").required(false).endParam()
            .responseMessage().code(200).message("OK").endResponseMessage()
            .responseMessage().code(500).message("error generating query").endResponseMessage()
